...
import com.cmcc.MainNLP;
import com.cmcc.impl.BaiduVoice;
import com.cmcc.impl.JiuTianVoice;
 ...
 * ------------------------------------------------------<br>
 * 1.0		2018年8月8日 下午3:17:38		zhouqiang		created.<br>
 */


public class Recorder extends JFrame implements MouseListener{
	...
	public static void main(String[] args) {
		...
		//语音解析类实例
		VoiceStrategy strategy = new BaiduVoice();
		//创造一个实例
		Recorder mr = new Recorder(strategy);
		
		
	}
	//构造函数
	public Recorder(VoiceStrategy voiceStrategy)
	{
		//语音解析策略初始化
		this.voiceStrategy = voiceStrategy;
		this.nlp = new MainNLP();
		//组件初始化
		jp1 = new JPanel();
		jp2 = new JPanel();
		jp3 = new JPanel();
		...
		//定义jp1的字体
		Font jpFont = new Font("华文新魏",Font.BOLD,20);
		jl1 = new JLabel("小区性能指标语音查询系统");
		jl1.setFont(jpFont);
		jl1.setForeground(Color.blue);
		jp1.add(jl1);
		//定义按钮上面的字体
		Font btFont = new Font("华文新魏",Font.BOLD,40);
		captureBtn = new JButton("按住说话查询");
	...
	public void mousePressed(MouseEvent e) {
		//当开始录音按钮被按下时就开始录音
		if(e.getSource().equals(captureBtn) && captureBtn.isEnabled())
		{
	        //改变按钮上面的字的内容
	        captureBtn.setText("查询中");
	...
	public void mouseReleased(MouseEvent e) {
		...
		if(e.getSource().equals(captureBtn) && captureBtn.isEnabled() )
		{
			...
			//jp2.repaint();
			//改变按钮上面的字的内容
			captureBtn.setText("按住说话查询");
			//调用保存录音的方法
			String resfilepath = save();
			captureBtn.setEnabled(false);
			String text = voiceStrategy.transformVoice2Text(resfilepath);
			//String text = "查询上周一至周五福州鼓楼区的平均无线接通率";
			System.out.println("text:"+text);
			jl1.setText(text);

			try {
				HashMap<String, String> map = nlp.translateNLP(text);
				String res = map.get("query_sql");
				if(res == null || "".equals(res)){
					res = "暂无结果。";
				}
				jl1.setText(res);
				System.out.println("RRR "+res.replace("。", ""));
			} catch (Exception e2) {
				e2.printStackTrace();
			}
	...
	public String save()
	{
        ...
        try {
        	...
        	res_audio_path = filePath+"/"+"test.wav";
        	file = new File(res_audio_path);
