...
public class XunFeiVoice implements VoiceStrategy
{
	// 原始音频存放地址
	//private static final String local_file = "D:\\语音查询.mp3";
	/*
	 * 转写类型选择：标准版和电话版分别为：
	...
	public String transformVoice2Text(String audioFilePath){
		...
		System.out.println("Get task result...");
		Message resultMsg;
		String res = "";
		// 获取任务结果
		try {
		...
		try {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				sb.append(jsonArray.getJSONObject(i).getString("onebest"));
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("return json error: "+res);
		}
		return sb.toString();
	}
	
	public static void main(String[] args) {
		System.out.println(new XunFeiVoice().transformVoice2Text("F:\\语音文件\\test.wav"));
	}
}
